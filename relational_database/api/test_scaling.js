...
async function runTests() {
    ...
    try {
        ...
        const writeRes = await axios.post(`${API_URL}/api/users`, {
        ...
        const readRes = await axios.get(`${API_URL}/api/users`);
        ...
        const shard2Res = await axios.post(`${API_URL}/api/shards/posts`, {
            user_id: 1, // 1%2 is 1 -> +1 = 2 (Shard 2)
            content: 'This is a post on Shard 2'
        });
        console.log('Response:', shard2Res.data);

        console.log('Testing User 2 (Should hit Shard 1)');
        const shard1Res = await axios.post(`${API_URL}/api/shards/posts`, {
            user_id: 2, // 2%2 is 0 -> +1 = 1 (Shard 1)
        ...
        const getShard2Res = await axios.get(`${API_URL}/api/shards/posts/1`);
        console.log('Response:', getShard2Res.data);

        console.log('Fetching User 2 Posts (Should read from Shard 1)');
        const getShard1Res = await axios.get(`${API_URL}/api/shards/posts/2`);
