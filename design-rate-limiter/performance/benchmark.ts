...
type RateLimiterInstance = LeakyBucket | FixedWindow | SlidingWindow;

const algorithms: Record<string, RateLimiterInstance> = {
...
interface ScenarioResults {
  [algorithmName: string]: BenchmarkResults;
}
...
async function runBenchmarks(): Promise<void> {
  ...
  const allResults: Record<string, ScenarioResults> = {};
  ...
  for (const scenario of scenarios) {
    ...
    for (const [name, algorithm] of Object.entries(algorithms)) {
      const results = await benchmarkAlgorithm(name, algorithm, scenario.requests, scenario.burstSize);
