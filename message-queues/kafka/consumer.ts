...
// ─────────────────────────────────────────────────────────────────────────────
const eventHandlers: Record<string, (p: EventPayload) => string> = {
    'user.signup': (p) => `👤 New ${String(p.plan ?? '?').toUpperCase()} user: ${p.email}`,
    ...
    'order.delivered': (p) => `✅ Delivered #${p.orderId} at ${String(p.deliveredAt ?? '').slice(0, 19)}`,
...
interface Stats {
    received: number;
    committed: number;
    byPartition: Record<number, number>;
...
function updateStats(partition: number, eventType: string): void {
    stats.received++;
    stats.committed++;
...
async function main(): Promise<void> {
    ...
    try {
        ...
        consumer.on('consumer.group_join', ({ payload }) => {
            ...
            for (const assignment of (payload.memberAssignment as Record<string, number[]>)[TOPIC_NAME] ?? []) {
        ...
        await consumer.run({
            ...
            eachMessage: async ({ topic: _topic, partition, message, heartbeat }) => {
                ...
                try {
                    parsed = JSON.parse(value) as ParsedMessage;
                } catch {
                    log('ERR', `Partition ${partition}@${offset} — invalid JSON, skipping`);
                    return;
                }

                const { id: _id, eventType, payload } = parsed;
