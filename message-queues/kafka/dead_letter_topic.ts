 ...
 *
 * Run: npx tsx dead_letter_topic.ts
 */
...
const TEST_MESSAGES: (Order | null)[] = [
...
async function main(): Promise<void> {
    ...
    try {
        ...
        for (const [topic, parts] of [[MAIN_TOPIC, 3], [DLT_TOPIC, 1]] as [string, number][]) {
            if (!existing.includes(topic)) {
                await admin.createTopics({
                    topics: [{
                        ...
                        replicationFactor: 1,
                        configEntries: [
                            { name: 'retention.ms', value: String(1000 * 60 * 60 * 24 * 30) },
                        ],
                    }],
                });
        ...
        await new Promise<void>((resolve) => {
            ...
            consumer.run({
                ...
                eachMessage: async ({ topic: _topic, partition, message }) => {
                    ...
                    try {
                        order = JSON.parse(rawValue) as Order;
                    } catch (e) {
                        ...
                        for (let attempt = 1; attempt <= MAX_RETRY; attempt++) {
                            try {
                                ...
                                if (attempt < MAX_RETRY) {
                                    ...
                                    log('RTRY', `Attempt ${attempt}/${MAX_RETRY} failed: ${lastError.message}`, `retry in ${backoff}ms`);
                                    results.retried++;
                                    await sleep(backoff);
                                }
                            }
        ...
        await new Promise<void>((resolve) => {
            ...
            dltConsumer.run({
                ...
                eachMessage: async ({ message }) => {
                    const reason = (message.headers['failure-reason'] as Buffer | undefined)?.toString() ?? 'unknown';
                    const origTopic = (message.headers['original-topic'] as Buffer | undefined)?.toString();
                    const origPart = (message.headers['original-partition'] as Buffer | undefined)?.toString();
                    const origOff = (message.headers['original-offset'] as Buffer | undefined)?.toString();
                    const failedAt = (message.headers['failed-at'] as Buffer | undefined)?.toString();
                    const retries = (message.headers['retry-count'] as Buffer | undefined)?.toString();
