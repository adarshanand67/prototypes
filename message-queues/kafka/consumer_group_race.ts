 ...
 *
 * Run: npx tsx consumer_group_race.ts
 */
...
interface ConsumerProgress {
    received: number;
    partition: number | null;
    processing: boolean;
}

const progress: Record<string, ConsumerProgress> = {
...
function renderRaceProgress(label: string = ''): void {
    const width = 40;
...
async function main(): Promise<void> {
    ...
    const partDistrib: Record<number, number> = { 0: 0, 1: 0, 2: 0 };
    ...
    const consumerResults = await Promise.all([0, 1, 2].map(async (idx): Promise<RaceResult> => {
        ...
        consumer.on('consumer.group_join', ({ payload }) => {
            const partitions = (payload.memberAssignment as Record<string, number[]>)[TOPIC] ?? [];
